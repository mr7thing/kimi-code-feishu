...
- **💬 飞书任务模式（默认）**：直接发消息 = 派 `kimi -p` 任务。会话落在包安装路径（npm 安装目录），默认 `kimi -c` 长会话续接（有上下文记忆，和新开的 kimi 会话一致），`/new` 新开
- **⌨️ 终端模式（/a 绑定后）**：直接发消息 = 自动注入绑定的终端会话（无需 /t）；`/task` 才派飞书任务；`/a free` 或 `/a 0` 释放切回。`/status` 随时显示当前模式

dashboard 里两者完全分开：「📡 本地会话」tab 只有终端会话转录（wire.jsonl），「💬 飞书对话」tab 是飞书消息与桥任务输出。

### 飞书里（手机/电脑均可）

| 输入 | 作用 |
|---|---|
| `帮我跑一下测试并修复失败用例` | 飞书任务模式：派 `kimi -p` 任务（流式回报，危险操作弹审批卡片） |
| 绑定终端后发普通消息 | 终端模式：自动注入该会话（不需要 /t） |
| `/task <文本>` | 终端模式下显式派发飞书任务 |
| `/bind /home/me/projects/foo` | 绑定本聊天的工作目录（飞书任务默认落在包安装路径） |
| `/a` | 列出终端会话；`/a 序号` 绑定进入终端模式；`/a free` 或 `/a 0` 释放切回 |
| `/t <文本>` | 显式注入文本+回车（终端模式下一般用不到） |
| `/i <文本>` | **优先插话**：绑定会话=等同 Ctrl+S 立即插入运行轮次；运行中任务=打断带前缀重启 |
| `/s` | 查看绑定会话：tmux=实时抓屏；pts=读磁盘转录尾部（最近对话） |
| `/c` | 审批池列表；`/c 序号` 或 `/c 路径` 切换（进池的终端会话才弹审批卡/推进度） |
| `/d`（=/dashboard） | 临时开启实时输出面板（默认本地链接；`/d public` 开公网隧道；`/d off` 关闭） |
| `/new` / `/stop` / `/status` / `/id` / `/help` | 新开会话 / 终止 / 状态（含模式）/ 查 open_id / 帮助 |
...
- **页面只读**，唯一操作是「关闭 Dashboard」按钮；SSE 断开时显示断连横幅（不会装死空白页）
- 每次开启：**新随机 token**；`/dashboard public` 才拉 cloudflared quick tunnel（**新随机域名**），上次的公网链接全部作废；默认本地模式不产生任何公网暴露
...
- **⏳ 待你处理**：待审批（工具+命令）与待回答提问（原文+选项），标注怎么回（飞书卡片 / `/t`）
- **🖥 终端会话（实时画面，2s 刷新）**：tmux 会话内嵌 `capture-pane` 实时屏幕——和 `tmux attach` 看到的一样；pts 会话标注「仅注入」
- **🏃 任务**：进行中的任务与耗时
- **💬 对话与事件**：飞书消息（👤 绿）/ 桥回复（🤖 蓝）/ 任务输出 / 审批进度
- **📡 本地会话转录（紫色）**：桥监听 `~/.kimi-code/sessions/**/wire.jsonl`（kimi 实时会话转录），把任何会话（**含普通终端 pts**）的用户输入、思考、回复、工具调用与结果流式推送——pts 屏幕读不到，但对话全在这里
