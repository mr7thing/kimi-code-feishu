...
终端里的交互式 kimi 会话可以交给你远程接管：会话跑在 tmux 里，飞书就是它的远程键盘。

```bash
kimi-code-feishu tmux     # 在 tmux 里启动 kimi（kcf-* 命名），Ctrl+B D 脱离
```

- 飞书发 `/a` 列出所有活会话（序号/目录），`/a 2` 绑定到本聊天；`/s` 看一眼屏幕再决定敲什么
...
node dist/selfcheck.js # 65 项端到端自检（假通道 + 假 kimi，不需要真实飞书）
