...
- **模型可以提问**（交互模式）——`AskUserQuestion` 会弹出飞书选项卡片，点选后真实按键作答
- 会话落在包安装路径（可用 `/bind` 改）；`/new` 杀掉重开；`/stop` 发 Esc 中断；`/i` 用 Ctrl+S 优先插话

**终端模式**（`/a 序号` 绑定任何其他终端会话后）：纯文本自动注入那个会话；`/0 <文本>` 发回 0 号会话；`/a free` 或 `/a 0` 切回。终端的 `/命令`（/model /yolo…）一律 `/t /model` 注入。

### 飞书里（手机/电脑均可）

| 输入 | 作用 |
|---|---|
| `帮我跑一下测试并修复失败用例` | 发给 0 号会话（常驻交互式 kimi，进度滚动回报，危险操作弹审批卡片，模型提问弹答题卡） |
| `/0 <文本>` | 终端模式下发回 0 号会话（飞书本身的常驻会话） |
| `/bind /home/me/projects/foo` | 绑定 0 号会话的工作目录（默认包安装路径） |
| `/a` | 列出终端会话；`/a 序号` 绑定进入终端模式；`/a free` 或 `/a 0` 切回 |
| `/t <文本>` | 注入绑定会话（终端的 `/命令` 也走这里：`/t /model`） |
| `/i <文本>` | **优先插话**（Ctrl+S，立即插入运行中的轮次） |
| `/s` | 查看绑定会话：tmux=实时抓屏；pts=读磁盘转录尾部（最近对话） |
| `/c` | 审批池列表；`/c 序号` 或 `/c 路径` 切换（进池的终端会话才弹审批卡/推进度） |
| `/d`（=/dashboard） | 实时输出面板（默认本地链接；`/d public` 公网隧道；`/d off` 关闭） |
| `/new` / `/stop` / `/status` / `/id` / `/help` | 杀掉 0 号会话重开 / Esc 中断 / 状态（含模式）/ 查 open_id / 帮助 |
