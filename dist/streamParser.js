 ...
 */
function* iterTextBlocks(obj) {
    if (typeof obj === 'string') {
        yield obj;
    }
    else if (Array.isArray(obj)) {
        for (const item of obj)
            yield* iterTextBlocks(item);
    }
...
function looksLikeToolEvent(obj) {
    ...
    if (Array.isArray(obj.tool_calls) && obj.tool_calls.length > 0) {
        const names = obj.tool_calls
            .map((tc) => String(tc.function?.name ?? tc.name ?? '?'))
            .join(', ');
...
export function parseLine(line) {
    ...
    if (obj && typeof obj === 'object' && !Array.isArray(obj)) {
        ...
        if (toolEv)
            return { kind: toolEv.kind, tool: toolEv.summary };
        const texts = [...iterTextBlocks(rec)].filter((t) => t.trim());
        if (texts.length)
            return { kind: 'text', text: texts.join('') };
        if (['done', 'result', 'finished'].includes(String(rec.type)))
            return { kind: 'done' };
        return null;
    }
    if (typeof obj === 'string')
        return { kind: 'text', text: obj };
    return null;
}
