...
export function bridgeBaseUrl(cfg) {
...
export function loadConfig(configPath) {
    ...
    let data = {};
    if (fs.existsSync(p)) {
        data = parseToml(fs.readFileSync(p, 'utf-8'));
    }
    ...
    // 环境变量覆盖（密钥优先走环境变量）
    if (process.env.KCF_APP_ID)
        cfg.appId = process.env.KCF_APP_ID;
    if (process.env.KCF_APP_SECRET)
        cfg.appSecret = process.env.KCF_APP_SECRET;
    if (process.env.KCF_ALLOWED_USER_IDS) {
        cfg.allowedUserIds = process.env.KCF_ALLOWED_USER_IDS.split(',').map((s) => s.trim()).filter(Boolean);
    }
    if (process.env.KCF_WORK_DIR)
        cfg.workDir = process.env.KCF_WORK_DIR;
    if (process.env.KCF_BRIDGE_PORT)
        cfg.bridgePort = Number(process.env.KCF_BRIDGE_PORT);
    if (process.env.KCF_KIMI_BIN)
        cfg.kimiBin = process.env.KCF_KIMI_BIN;
    return cfg;
}
export function saveExampleConfig(configPath) {
    const p = configPath ?? configPathFromEnv();
    if (fs.existsSync(p))
        return p;
