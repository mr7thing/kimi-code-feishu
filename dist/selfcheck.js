...
class FakeChannel {
    ...
    async updateText(messageId) { this.updated.push(messageId); }
    async updateCard(messageId) { this.updated.push(messageId); }
    cards() {
        return this.sent.filter((s) => s.kind === 'card').map((s) => s.content);
    }
    texts() {
        return this.sent.filter((s) => s.kind === 'text').map((s) => s.content);
    }
}
...
async function runHook(cfgPath, event, payload) {
    ...
    // ---------------------------------------------------------------- 5. 端到端：审批回环
    {
        ...
        const approver = (async () => {
            for (let i = 0; i < 50; i++) {
                ...
                if (cards.length >= 1) {
                    const actions = cards[0].elements[2].actions;
                    await sleep(100);
                    bridge.onCardAction(actions[0].value, 'ou_boss');
                    return;
                }
                await sleep(100);
            }
        ...
        const denier = (async () => {
            for (let i = 0; i < 50; i++) {
                const cards = channel.cards();
                if (cards.length >= 2) {
                    const actions = cards[1].elements[2].actions;
                    await sleep(100);
                    bridge.onCardAction(actions[2].value, 'ou_boss'); // 拒绝按钮
                    return;
                }
                await sleep(100);
            }
        ...
        const evil = (async () => {
            for (let i = 0; i < 50; i++) {
                const cards = channel.cards();
                if (cards.length >= 3) {
                    const actions = cards[2].elements[2].actions;
                    bridge.onCardAction(actions[0].value, 'ou_hacker');
                    return;
                }
                await sleep(100);
            }
        ...
        await evil;
        check('端到端: 非白名单点击被忽略且超时拒绝', isDeny(r.stdout) && Date.now() - t0 >= cfg.approvalTimeout * 1000 - 1000);
        // 5.6 进度事件 → 飞书进度消息
...
`, 'utf-8');
        ...
        let done = false;
        for (let i = 0; i < 60; i++) {
            if (channel.texts().some((t) => t.includes('任务完成') && t.includes('一切正常'))) {
                done = true;
                break;
            }
            await sleep(200);
