...
function denyJson(reason: string): void {
...
async function post(url: string, body: unknown, timeoutMs: number): Promise<Record<string, unknown>> {
...
async function main(): Promise<number> {
  ...
  if (process.env.KCF_DISABLED) return 0; // 紧急旁路：让 CLI 回到原生行为
